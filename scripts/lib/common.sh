...
load_runtime_env() {
    [ -f "$RUNTIME_ENV" ] || die "$RUNTIME_ENV missing - provisioning has not completed. Run: colossul provision"
    # shellcheck disable=SC1090
    source "$RUNTIME_ENV"
...
write_extra_model_paths() {
    ...
    local -a _d
    {
        ...
        echo "# Regenerate with: colossul provision"
        echo "#"
...
print_model_status() {
    ...
    if [ "$running" = "1" ]; then
        ...
        echo ""
        echo "      Follow it:   colossul models-log"
        echo "      (also in the Instance Portal -> Logs -> models)"
    elif [ "${n:-0}" -gt 0 ]; then
        echo "      $n weight file(s) in $root"
        echo "      Add more:  colossul models --list"
    else
        echo "      NONE DOWNLOADED — ComfyUI will show \"Missing Models\" on any"
        echo "      workflow that needs weights."
        echo ""
        echo "      See what is available:   colossul models --list"
        echo "      Download a set now:      colossul models minimax-h3"
...
print_access_summary() {
    ...
    echo "      - Admin:  colossul status | urls | logs <n> | restart <n>"
    echo ""
    echo "=================================================================="
    echo ""
}
...
write_seat_unit() {
...

    {
        ...
        echo "; Generated by provision.sh - regenerate with: colossul provision"
...
write_models_unit() {
    local outfile="$1" sets="$2"
    {
        ...
        echo "; Progress:  colossul models-log     Re-run:  colossul models $sets"
        echo ""
